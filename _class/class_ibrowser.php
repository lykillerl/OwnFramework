...
#Create by LYK on 2013-11-11 @ 14:00PM
class IBrowser
{
    ...
    public function setHttpHeaders($httpHeaders = null)
    {
        ...
        foreach ($httpHeaders as $key => $value)
            if (substr($key, 0, 5) == 'HTTP_') $this->httpHeaders[$key] = $value;
    }
    ...
    public function setUserAgent($userAgent = null)
    {
        ...
        else
        {
            ...
            foreach ($this->getUaHttpHeaders() as $altHeader)
                if (!empty($this->httpHeaders[$altHeader])) $this->userAgent .= $this->httpHeaders[$altHeader]." ";
    ...
    public function getMobileDetectionRulesExtended()
    {
        static $rules;
        if (!$rules) $rules = array_merge(self::$phoneDevices, self::$tabletDevices, self::$operatingSystems,
                self::$browsers, self::$utilities);
        return $rules;
    }

    public function getRules()
    {
        if ($this->detectionType == self::DETECTION_TYPE_EXTENDED) return self::
            ...
            if (isset($this->httpHeaders[$mobileHeader]))
                if (is_array($matchType['matches']))
                {
                    foreach ($matchType['matches'] as $_match)
                        if (strpos($this->httpHeaders[$mobileHeader], $_match) !== false) return true;
                }
                else  return true;
        return false;
    }
    ...
    public function version($propertyName, $type = self::VERSION_TYPE_STRING)
    {
        ...
        if ($type != self::VERSION_TYPE_STRING && $type != self::VERSION_TYPE_FLOAT) $type = self::
                VERSION_TYPE_STRING;
    ...
    public function mobileGrade()
    {
        ...
        if ( // Apple iOS 3.2-5.1 - Tested on the original iPad (4.3 / 5.0), iPad 2 (4.3), iPad 3 (5.1), original iPhone (3.1), iPhone 3 (3.2), 3GS (4.3), 4 (4.3 / 5.0), and 4S (5.1)
            $this->version('iPad', self::VERSION_TYPE_FLOAT) >= 4.3 || $this->version('iPhone', self::
            VERSION_TYPE_FLOAT) >= 3.1 || $this->version('iPod', self::VERSION_TYPE_FLOAT) >= 3.1 ||
        ...
        $this->is('BlackBerry') && $this->version('BlackBerry', self::VERSION_TYPE_FLOAT) >= 6.0 ||
            ...
            $this->match('Playbook.*Tablet') || // Palm WebOS (1.4-2.0) - Tested on the Palm Pixi (1.4), Pre (1.4), Pre 2 (2.0)
            ($this->version('webOS', self::VERSION_TYPE_FLOAT) >= 1.4 && $this->match('Palm|Pre|Pixi')) ||
            // Palm WebOS 3.0  - Tested on HP TouchPad
            $this->match('hp.*TouchPad') || // Firefox Mobile (12 Beta) - Tested on Android 2.3 device
            ($this->is('Firefox') && $this->version('Firefox', self::VERSION_TYPE_FLOAT) >= 12) ||
            // Chrome for Android - Tested on Android 4.0, 4.1 device
            ($this->is('Chrome') && $this->is('AndroidOS') && $this->version('Android', self::
            VERSION_TYPE_FLOAT) >= 4.0) || // Skyfire 4.1 - Tested on Android 2.3 device
        ...
        $this->is('Dolfin') && $this->version('Bada', self::VERSION_TYPE_FLOAT) >= 2.0 ||
            ...
            (($this->is('UC Browser') || $this->is('Dolfin')) && $this->version('Android', self::
            VERSION_TYPE_FLOAT) >= 2.3) || // Kindle 3 and Fire  - Tested on the built-in WebKit browser for each
            ($this->match('Kindle Fire') || $this->is('Kindle') && $this->version('Kindle', self::
            VERSION_TYPE_FLOAT) >= 3.0) || // Nook Color 1.4.1 - Tested on original Nook Color, not Nook Tablet
        ...
        $this->version('Opera', self::VERSION_TYPE_FLOAT) >= 10 && !$isMobile) return self::MOBILE_GRADE_A;

        if ($this->version('iPad', self::VERSION_TYPE_FLOAT) < 4.3 || $this->version('iPhone', self::
            VERSION_TYPE_FLOAT) < 3.1 || $this->version('iPod', self::VERSION_TYPE_FLOAT) < 3.1 ||
            // Blackberry 5.0: Tested on the Storm 2 9550, Bold 9770
            $this->is('Blackberry') && $this->version('BlackBerry', self::VERSION_TYPE_FLOAT) >= 5 && $this->
            version('BlackBerry', self::VERSION_TYPE_FLOAT) < 6 ||
            //Opera Mini (5.0-6.5) - Tested on iOS 3.2/4.3 and Android 2.3
            ($this->version('Opera Mini', self::VERSION_TYPE_FLOAT) >= 5.0 && $this->version('Opera Mini', self::
            VERSION_TYPE_FLOAT) <= 6.5 && ($this->version('Android', self::VERSION_TYPE_FLOAT) >= 2.3 || $this->
            is('iOS'))) || // Nokia Symbian^3 - Tested on Nokia N8 (Symbian^3), C7 (Symbian^3), also works on N97 (Symbian^1)
            $this->match('NokiaN8|NokiaC7|N97.*Series60|Symbian/3') ||
            // @todo: report this (tested on Nokia N71)
            $this->version('Opera Mobi', self::VERSION_TYPE_FLOAT) >= 11 && $this->is('SymbianOS')) return self::
                MOBILE_GRADE_B;

        if ( // Blackberry 4.x - Tested on the Curve 8330
            $this->version('BlackBerry', self::VERSION_TYPE_FLOAT) < 5.0 ||
            // Windows Mobile - Tested on the HTC Leo (WinMo 5.2)
            $this->match('MSIEMobile|Windows CE.*Mobile') || $this->version('Windows Mobile', self::
            VERSION_TYPE_FLOAT) <= 5.2) return self::MOBILE_GRADE_C;
